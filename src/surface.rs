...
impl Surface {
    pub fn create(properties: SurfaceProperties, state: &mut WaylandState, queue_handle: &QueueHandle<WaylandState>) -> Option<Surface> {
        ...
        let surface = Surface {
            properties,
            interfaces: SurfaceInterfaces {
                wayland_surface, layer_surface, gpu_interface
            },
        };

        surface.interfaces.layer_surface.set_margin(
            surface.properties.margins.top,
            surface.properties.margins.right,
            surface.properties.margins.bottom,
            surface.properties.margins.left,
        );
        surface
            .interfaces
            .layer_surface
            .set_anchor(surface.properties.anchor);
        surface
            .interfaces
            .layer_surface
            .set_keyboard_interactivity(surface.properties.interactivity);
        surface.interfaces.layer_surface.set_size(
            surface.properties.sizes.width.get(),
            surface.properties.sizes.height.get(),
        );
        surface.interfaces.wayland_surface.commit();
