...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while display.is_alive() {
        ...
        if let Some(surface) = wayland_state.surface_links.get_mut(&surface_id) {
            // unsafe {surface.gpu_surface.resize(NonZero::new_unchecked(5), NonZero::new_unchecked(5));}
            println!("age: {} {} {} {}",
                surface.gpu_surface.surface.buffer_age(),
                surface.gpu_surface.surface.is_current(&surface.gpu_surface.context),
                surface.gpu_surface.surface.is_current_draw(&surface.gpu_surface.context),
                surface.gpu_surface.surface.is_current_read(&surface.gpu_surface.context)
                );
        }
        ...
        if let Some(surface) = wayland_state.surface_links.get_mut(&surface_id) {
            if !has_surface {
                ...
                println!("swapped!")
            } else if has_surface && !has_resized {
                has_resized = true;
